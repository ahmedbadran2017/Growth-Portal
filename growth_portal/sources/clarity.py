...
⚠️ **Not verified against the live project, and the reason is known.** The Data
Export API needs its own token, generated by a project admin at
Settings → Data Export → Generate new API token. That is a different credential
from the OAuth session the Clarity MCP uses — which is why session recordings
answer on this project while every aggregate query returns an error. It is not
the daily quota.

So the adapter stays out of `tasks.SOURCES` until `clarity_api_token` is set and
a probe returns rows. A registered adapter that has never worked would report a
source as live on the health strip.
...
* **Ten requests per day, per project.** The dimension list is a budget, not a
  wish list.
* **1,000 rows per response, and no pagination.** This is why each call asks for
  a single dimension even though the API accepts three: crossing URL with Device
  and Source multiplies the row count and the truncation would be silent — the
  response gives no indication it was cut.
...
class ClaritySource(SourceAdapter):
    name = "clarity"
    #: Documented: "The API returns results in UTC timezone" — regardless of
    #: what the project or the site is set to.
    timezone = "UTC"
    maturity_hours = 24
    ...
    def _get(self, num_days, dimension):
        ...
        if r.status_code in (401, 403):
            raise RuntimeError(
                "Clarity rejected the token. The Data Export API needs a token "
                "generated at Settings -> Data Export, which is not the same "
                "credential as a Clarity login."
            )
    ...
    def metrics(self, date_from: date, date_to: date):
        ...
        """
        # Clarity reports UTC; the rest of this app runs on the site clock.
        # The window is small enough that the difference decides which day a
        # row lands on.
        today = guard.site_today()
